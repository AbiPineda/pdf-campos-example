...
export class DocumentsComponent {
  ...
  async modifyPdf() {
    try {
      ...
      // Cargar un documento PDF desde los bytes PDF existentes
      ...
      // Obtener el ancho y el alto de la primera página
      const { width, height } = firstPage.getSize();

      //creamos formulario
      const form = pdfDoc.getForm();

      //Agrego el input del nombre
      const nameField = form.createTextField('name.fields');
      ...
      const emailField = form.createTextField('mail.fields');
      ...
      const datelField = form.createTextField('date.fields');
      datelField.addToPage(firstPage, {
        x: 112,
        y: 698,
        width: 250,
        height: 15,
